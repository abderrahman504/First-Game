...
func get_new_gun(GunType : int):
	...
	var newGun : Node
	if GunType == GlobalReferences.GUNTYPES.Machinegun:
		newGun = load(GlobalReferences.GunPaths["machinegun"]).instance()
	elif GunType == GlobalReferences.GUNTYPES.Shotgun:
		newGun = load(GlobalReferences.GunPaths["shotgun"]).instance()
	elif GunType == GlobalReferences.GUNTYPES.SplitRifle:
		newGun = load(GlobalReferences.GunPaths["split rifle"]).instance()
	elif GunType == GlobalReferences.GUNTYPES.HeavyCanon:
